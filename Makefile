...
uninstall: ## uninstall the package
	pip uninstall -y $(PIP_ARGS) meme_generator || true
...
env-create: init-hook ## (re)create a development environment using tox
	...
	./.tox/meme_generator/bin/pip install pip==20.2.4
...
lint: ## static code analysis with pylint
	pylint --rcfile meme_generator/.pylintrc --disable=$(EXTRA_DISABLE) -j 0 meme_generator
...
coverage: ## check test coverage
	coverage run --source meme_generator -m pytest
...
security: ## check source code for vulnerabilities or dangerous code
	bandit -c .bandit.yml -v -r meme_generator

security-report: ## check source code for vulnerabilities and generate an HTML report
	@mkdir -p docs/_build/security
	bandit -c .bandit.yml -v -r -f html -o docs/_build/security/index.html meme_generator
...
fmt: ## format code using the PEP8 convention
	black meme_generator

docs: ## generate project docs
	rm -f docs/meme_generator.rst
	find docs -name 'meme_generator.*.rst' -exec rm -f {} +
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ meme_generator
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
doc-test-plan: dir-health
	...
	@sed -i -e 's/project_name/meme_generator/g' docs/health/test_plan.rst
...
doc-release: dir-operations
	...
	@sed -i -e 's/project_name/meme_generator/g' docs/operations/release.rst
	@echo Release created!

doc-run: dir-operations
	@cp -n ~/projects/daskeleton/templates/operations/run.rst docs/operations/run.rst
	@sed -i -e 's/project_name/meme_generator/g' docs/operations/run.rst
	@echo Run created!
...
doc-usage: dir-support
	...
	@sed -i -e 's/project_name/meme_generator/g' docs/support/usage.rst
	@sed -i -e 's/project_name/meme_generator/g' docs/support/usage.rst
	@echo Usage created!
...
publish-dev: ## publish packages to the repository.
	...
	twine upload --repository dasnano dist/meme_generator*
...
docker-run: ## run the specified command on docker (defaults to /bin/bash)
	docker-compose -f docker/dev/docker-compose.yml run --rm -u $(CURRENT_USER) meme_generator $(MAKE) $(TARGET)

docker-shell: ## drop the user into a shell inside a docker container
	docker-compose -f docker/dev/docker-compose.yml run --rm -u $(CURRENT_USER) meme_generator
...
ci-dist-dev: ## build the package (for development) for every supported version on CI (GitLab)
	tox -e meme_generator -- $(MAKE) dist-dev

ci-publish: ## publish built packages to the appropriate repository on CI (GitLab)
	tox -e meme_generator -- $(MAKE) publish ## publish the packages generated with dist or dist-dev on CI (GitLab)
